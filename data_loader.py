...
# Set option for wich dataset want to train
dataset = 'cub' #

need_256 = True # set to True for stackGAN
datapath = str(Path.home()) + '/resources/'
...
if dataset == 'cub':
    ...
    captions_ids = [80000, any]
    """
    cwd = datapath + 'cub2002011/'
    img_dir = os.path.join(cwd, 'cub2002011')
    caption_dir = os.path.join(cwd, 'text_c10')
    VOC_FIR = cwd + '/vocab.txt'
    ...
    for sub_dir in caption_sub_dir: # get caption file list
        with tl.ops.suppress_stdout():
            files = tl.files.load_file_list(path=sub_dir, regx='.txt')
            for i, f in enumerate(files):
                file_dir = os.path.join(sub_dir, f)
                key = int(re.findall('\d+', f)[0])
                t = open(file_dir,'r')
    ...
    ## load images
    with tl.ops.suppress_stdout():  # get image files list
        # Cub dataset is ordered in sub folders so retrieve images for each
        # sub folder
        imgs_title_list = []
        for sub_dir in load_folder_list(img_dir):
            imgs_title_list += [os.path.join(sub_dir, img)
                for img in sorted(tl.files.load_file_list(path = sub_dir, regx = '.jpg'))]
    ...
    try:
        for name in imgs_title_list:
            # print(name)
            img_raw = scipy.misc.imread( os.path.join(img_dir, name) )
            # Image is black and white, convert to "rgb"
            if img_raw.ndim == 2:
